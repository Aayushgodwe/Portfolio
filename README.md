# 🌐 Personal Portfolio Website

## 👨‍💻 About Me

Hi, I’m **Aayush Godwe**, a passionate engineering student with an interest in **AI, software development, and web technologies**.
This portfolio showcases my skills, projects, and achievements.

---

## 🚀 About the Project

This is a **responsive personal portfolio website** built using **HTML and CSS**.
It highlights my profile, education, technical skills, and projects in a clean and modern design.

---

## ✨ Features

* 📱 Fully responsive design (mobile + desktop)
* 🎨 Clean and modern UI
* 📌 Easy navigation with sticky navbar
* 📂 Organized sections:

  * Home
  * About
  * Education
  * Skills
  * Projects
  * Certifications & Achievements
  * Contact

---

## 🛠️ Technologies Used

* HTML5
* CSS3

---

## 📂 Project Structure

```
portfolio/
│── index02.html
│── profile_pic.jpeg
│── portfoliobc.jpeg
│── portfoliobc_hd.jpeg
```

---

## 📸 Preview

(Add screenshot here if needed)

---

## 💡 Projects Included

* 🏠 Hostel Finder Website
* 🌐 Portfolio Website
* 🤖 AI Chatbot

---

## 🎓 Education

* B.Tech in Computer Science (AIML)
* MIT Academy of Engineering, Pune

---

## 🧠 Skills

* Python, C Programming
* HTML, CSS
* SQL, Power BI
* Excel, Canva

---

## 🏆 Certifications

* Digital 101 – NASSCOM
* CISCO Python Programming
* AI & Anthropic Courses
* Career Management Essentials (IBM)

---

## 🏅 Achievements

* Participated in hackathons
* Completed multiple certifications
* Active in tech communities
* Volunteer in Maths Poster Presentation

---

## 📞 Contact

* 📧 Email: [aayushgodwe11@gmail.com](mailto:aayushgodwe11@gmail.com)
* 📱 Phone: +91 9423841541
* 🔗 LinkedIn: https://www.linkedin.com/in/aayush-godwe-72a304397/
* 💻 GitHub: https://github.com/Aayushgodwe

---

## ⚙️ How to Run

1. Clone the repository

   ```bash
   git clone https://github.com/Aayushgodwe/Portfolio.git
   ```
2. Open the project folder
3. Run `index02.html` in your browser

---

## 🌟 Future Improvements

* Add JavaScript for interactivity
* Improve animations
* Deploy live website

---

## 📌 Note

This project is created for learning and portfolio purposes.

---

## ©️ 2026 Aayush Godwe
